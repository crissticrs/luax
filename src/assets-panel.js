...
// Uses projectAssets / projectMusic / scenes + LuaX APIs (gfx.sprite, sfx.music, gfx.scene)
...
function assetCodeSnippet(kind, name) {
    ...
    if (kind === 'scene') return 'gfx.scene("' + n + '")';
...
function collectProjectAssets() {
    ...
    try {
        ...
        // Scenes (2D levels) from scene editor storage
        let sceneMap = {};
        try {
            if (typeof getProjectSceneMap === 'function') {
                sceneMap = getProjectSceneMap(currentProjectName) || {};
            }
        } catch (_) {}
        if (!sceneMap || typeof sceneMap !== 'object' || !Object.keys(sceneMap).length) {
            try {
                const raw = localStorage.getItem('luax_project_scenes');
                const all = raw ? JSON.parse(raw) : {};
                sceneMap = (all && all[currentProjectName]) || {};
            } catch (_) { sceneMap = {}; }
        }
        Object.keys(sceneMap || {}).sort().forEach(name => {
            out.push({ kind: 'scene', name: name, thumb: null, category: 'Scenes' });
        });
    } catch (_) {}
    return out;
}
...
function filteredAssetsList() {
    ...
    if (assetsPanelState.filter === 'scenes') list = list.filter(a => a.kind === 'scene');
...
function renderAssetsPanel() {
    ...
    let html = '';
    ['Sprites', 'Music', 'Scenes'].forEach(cat => {
        ...
        items.forEach(a => {
            ...
            let thumb = a.kind === 'sprite' && a.thumb
                ...
                : (a.kind === 'music'
                    ...
                    : (a.kind === 'scene'
                        ? '<div class="assets-icon assets-icon-scene" aria-hidden="true">🗺</div>'
    ...
    if (!html) {
        html = '<div class="assets-empty">' +
            (currentProjectName
                ? 'No assets yet.<br><span class="assets-empty-hint">Add sprites, music, or Scene (2D) levels from the Files view.</span>'
...
function previewAsset(kind, name) {
    ...
    if (kind === 'sprite') {
        ...
        box.innerHTML =
            '<div class="assets-preview-head"><span>🗺 ' + escapeHtml(name) + '</span>' +
            '<button type="button" class="btn btn-sm" onclick="hideAssetsPreview()">✕</button></div>' +
            '<p class="assets-preview-meta">2D scene / level</p>' +
            '<button type="button" class="btn btn-primary btn-sm assets-preview-insert" data-kind="scene" data-name="' + escapeHtml(name) + '">Insert gfx.scene(…)</button>' +
            '<button type="button" class="btn btn-sm" onclick="openSceneEditor(\'' + String(name).replace(/'/g, "\\'") + '\', false)">Edit scene</button>';
    } else {
...
function showAssetsContextMenu(x, y, kind, name) {
    ...
    m.innerHTML =
        ...
        (kind === 'scene' ? '<button type="button" data-act="edit-scene">Edit scene</button>' : '') +
    ...
    m.addEventListener('click', (e) => {
        ...
        if (act === 'insert') insertAssetAtCursor(kind, name);
        ...
        else if (act === 'edit-scene' && typeof openSceneEditor === 'function') openSceneEditor(name, false);
        else if (act === 'delete') {
            ...
            if (kind === 'sprite') {
                ...
                try {
                    if (typeof deleteProjectScene === 'function') deleteProjectScene(name);
                    else if (typeof window.deleteProjectScene === 'function') window.deleteProjectScene(name);
                    else {
                        const raw = localStorage.getItem('luax_project_scenes');
                        const all = raw ? JSON.parse(raw) : {};
                        if (all[currentProjectName]) {
                            delete all[currentProjectName][name];
                            localStorage.setItem('luax_project_scenes', JSON.stringify(all));
                        }
                    }
                } catch (_) {}
            }
